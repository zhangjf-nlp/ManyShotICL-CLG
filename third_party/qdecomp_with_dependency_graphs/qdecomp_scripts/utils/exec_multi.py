...
def run(cmd):
    with open(os.devnull, 'wb') as devnull:
        ...
        try:
            ...
            #cmd = f'CUDA_VISIBLE_DEVICES={str(gpu)}; {cmd}'  #  doesnt cover python qdecomp_scripts
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run multiple commands on cuda pool.'
                                                 'example: PYTHONPATH=. nohup python qdecomp_scripts/utils/exec_multi.py --cuda 2 3 > exec/log.txt &')
