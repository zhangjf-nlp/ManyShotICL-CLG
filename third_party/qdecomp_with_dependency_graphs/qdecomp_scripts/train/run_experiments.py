...
def eval(model_dir:str, dataset_set:str=None, dataset_file:str = None, dataset_dir:str = None, predict_only: bool = False, dest_sub_dir: str = 'eval'):
    ...
    for predict_dataset, dataset_file, output_base in dataset_files_list:
        try:
            ...
            cmd_eval = f"""nohup python qdecomp_scripts/eval/evaluate_predictions.py --dataset_file={shlex.quote(metadata_file)} --preds_file={shlex.quote(preds_output_file)} --allennlp --output_file_base={shlex.quote(eval_output_file_base)} """
...
def tune(experiment: str, serialization_root_dir: str,
    ...
    cmd_cleanup = f'python qdecomp_scripts/train/run_experiments.py cleanup --dir {serialization_parent} --top 3'
    ...
    for conf in configs:
        ...
        cmd = f"nohup python qdecomp_scripts/train/run_experiments.py train --experiment {experiment} {dataset_part} --no_eval -s {serialization_root_dir} -o {' '.join(over)}"
...
def best(configs_dir: str,
    ...
    for path in Path(configs_dir).rglob('*.json'):
        cmd = f"nohup python qdecomp_scripts/train/run_experiments.py train --experiment {str(path)} {dataset_part} -s {serialization_root_dir}"
...
if __name__ == '__main__':
    ...
    cleanup_parser = subparser.add_parser('cleanup', help='cleanup tuning experiments (keeps top k, in terms of best_validation_loss). '
                                                          'example: python qdecomp_scripts/run_experiments.py cleanup --dir tmp/tune --top 3 ')
