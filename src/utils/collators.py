...
class DataCollatorWithPaddingAndCuda:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> BatchEncoding:
        ...
        type_valid = lambda v:type(v) not in [str, dict] and not (type(v) is list and type(v[0]) is str)
        features = [{k:v for k,v in f.items() if type_valid(v)} for f in features]
