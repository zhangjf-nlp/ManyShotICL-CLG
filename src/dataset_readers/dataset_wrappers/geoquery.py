...
def get_a(entry):
    a = entry['funql']
    assert a.startswith("answer(") and a.endswith(")")
    a = a[len("answer("):-len(")")]
    return a
...
@field_getter.add("qa")
def get_qa(entry):
    return f"{get_q(entry)}\tanswer: {get_a(entry)}"


@field_getter.add("gen_a")
def get_gen_a_instruction(entry):
    return "{ice_prompt}{question}\tanswer: ".format(question=entry['question'], ice_prompt='{ice_prompt}')
